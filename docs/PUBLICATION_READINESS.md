...
- [x] **Predefine scenarios and reserve fresh evaluation seeds.** Cross balanced, moderately skewed, and strongly skewed demand with below-capacity, near-capacity, and overloaded conditions, assessed per approach. Include changing demand and recovery. Changing which approach is busiest needs a plan extension; current skews stay fixed. Hypothesis: uneven/changing demand with greens away from both bounds may benefit; persistent overload may erase that benefit. Include neutral and adverse cases, and verify actual green-duration distributions rather than selecting only favorable runs.
  ...
  **Status:** frozen in [`scripts/scenarios.py`](../scripts/scenarios.py) and written up in [EXPERIMENT_PROTOCOL.md](EXPERIMENT_PROTOCOL.md) — three skews crossed with four regimes, development seeds 301–308, evaluation seeds 9001–9020 held back. Green-duration distributions are recorded per arm, against each controller's own bounds.

  **Corrected 10 September 2026, after review.** Three claims here were overstated:

  - The regimes were named below / near / over **capacity** from a single finite-workload clearance figure. Capacity has not been measured; they are now named for the rates they offer, and saturation status is a hypothesis, not a cell name.
  - "Pinning changes arrival times only, not the vehicle draws" holds **among pinned regimes**. The changing-demand cell does not share the pinned cells' vehicle sequence — at seed 301, N=500, they differ at 492 of 500 positions.
  - The grid was frozen with a scenario key that collapsed the regime axis, so the twelve cells would have produced three strata. Fixed; twelve cells now give twelve strata, each with its own safeguards.

  Still open: which approach is busiest cannot change mid-run, so "changing demand" means changing *total* demand only.
...
- [x] **Define the workload and inference before collection.** Choose N/horizon to cover several load transitions; specify initialization, arrival cutoff, draining, and timeout handling. Analyze finite-workload clearance separately from sustained-flow performance. A time-limited study needs residual-queue logging. Retain/report all failures by scenario and arm; do not hide difficult cases through exclusions. Use independent plan-level differences and 95% confidence intervals, stratified by scenario; choose replication count from pilot variability and desired precision. Vehicle-level p-values are unsuitable as the main evidence because vehicles interact; the batch analyzer already uses plan means. [Wilcoxon assumptions](https://docs.scipy.org/doc/scipy/reference/generated/scipy.stats.wilcoxon.html) still need checking.
  ...
  **Corrected 10 September 2026, after review.** The stratification was by skew and workload only, so it was not stratified by scenario at all; the replication-count function used an unrelated constant for every confidence level except 95%, returning 19 plans for a 99% request where its own formula gives 60; and nothing checked that pooled plans were independent replicates from one experiment — a copied plan folder counted as two plans. All three are fixed and tested. Residual-queue logging, which this item calls for, still does not exist: the discharge report now says so instead of naming a proxy after it.
...
- [x] **Extend logging and analysis:** record decision-time counts, round/phase identity, selected and actual green duration, green end, queue traces, and actual arrival/crossing times. Current green-onset logs cannot fully reconstruct the final phase or explain every decision.
  ...
  **Corrected 10 September 2026, after review.** This was marked done while the final phase of every run was still missing. The phase record was written only after the whole green and yellow had finished, and the controller is a daemon thread killed 1.5 s after the last crossing — so the phase that served the last vehicles never reached disk. All sixteen arms of the committed pilot have exactly one more signal-change row than phase row, and the crossings that green served were reported as crossings outside any green: 43 of them in `even_500_seed304/fixed`, 18 in `seed302/fixed`. A phase is now opened when it is decided and finalized by shutdown, marked censored; the analyzers check phase-log completeness and reject a log with greens missing from it. **The pilot's phase-derived numbers — green distributions, discharge, utilisation — need recollection under the fixed logging. Its vehicle-wait rows are unaffected and reanalyse to the same figures.**
