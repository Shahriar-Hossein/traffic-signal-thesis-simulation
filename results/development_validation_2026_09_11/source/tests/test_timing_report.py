...
class TimingReportTests(unittest.TestCase):
    ...
    def write_arm(self, name, release_shift=0.0, phase_shift=0.0, windows=(60, 60),
        ...
        (folder / 'run_meta.json').write_text(json.dumps({
            ...
            'fps_window_bounds': [[i * window_sec, (i + 1) * window_sec]
                                  for i in range(len(windows))],
            'fps_covered_sec': len(windows) * window_sec,
            'last_crossing_sec': len(windows) * window_sec - 1.5,
            'duration_sec': len(windows) * window_sec,
        }))
        return folder
